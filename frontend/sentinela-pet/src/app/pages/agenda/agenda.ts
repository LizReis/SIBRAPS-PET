...
export class Agenda implements OnInit, OnDestroy {
  ...
  readonly statusUi: Record<string, StatusUi> = {
    PRESENTE: { label: 'PRESENTE', classe: 'status-presente', patternClass: 'pattern-presente' },
    AGENDADO: { label: 'AGENDADO', classe: 'status-agendado', patternClass: 'pattern-agendado' },
    FALTOU: { label: 'FALTOU', classe: 'status-faltou', patternClass: 'pattern-faltou' },
    REMARCADO: { label: 'REMARCADO', classe: 'status-remarcado', patternClass: 'pattern-remarcado' },
  ...
  possuiAcoes(a: AgendamentoDTO): boolean { return this.podeRegistrarFrequencia(a) || this.podeRemarcar(a); }
  ...
  remarcar(a: AgendamentoDTO): void { this.router.navigate(['/agenda/novo'], { queryParams: { agendamentoOriginalId: a.id } }); }
  ...
  classeSituacao(s: string): string { return this.statusUi[s]?.classe ?? 'status-neutro'; }
  classePadraoSituacao(s: string): string { return this.statusUi[s]?.patternClass ?? 'pattern-neutro'; }
  labelSituacao(s: string): string { return this.statusUi[s]?.label ?? s; }
  descricaoSituacao(s: string): string | null { return this.statusUi[s]?.descricao ?? null; }
  ...
  private minutosDoHorario(h: string): number { const [hora, minuto] = this.formatarHora(h).split(':').map(Number); return hora * 60 + minuto; }
  ...
  formatarDataISO(data: Date): string { return `${data.getFullYear()}-${String(data.getMonth() + 1).padStart(2, '0')}-${String(data.getDate()).padStart(2, '0')}`; }
